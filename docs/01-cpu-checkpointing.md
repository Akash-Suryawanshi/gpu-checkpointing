...
On our restricted host, CRIU's initial CPU path fails during startup feature detection involving `kerndat_has_nftables_concat`. This does not mean the counter needs networking. Enabled user-namespace settings likewise did not make actual namespace creation succeed. Keep these dated permission results in [the evidence](../experiments/results.md), separate from the mechanism above.
...
On this host DMTCP's CPU counter restore passed. A complete PyTorch GPU process also restored after the original exited, but unresolved shared-memory warnings qualify that observation. A basic success under one mechanism does not establish unrestricted compatibility under either tool. Continue with [GPU checkpointing](02-gpu-checkpointing.md) to see what must be added to the CPU state.

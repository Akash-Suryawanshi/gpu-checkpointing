...
After GPU restoration, CUDA is unlocked and ordinary GPU work can continue. For the proposed training comparison, verify the restored state before allowing the next update. The existing DMTCP tensor probe releases its file wait earlier; its next CUDA call waits for the plugin to finish GPU restoration. Neither path uses `torch.load` to reconstruct training state from an application checkpoint. See [NVIDIA's restore description](https://github.com/NVIDIA/cuda-checkpoint#the-utility).
...
## What evidence would connect the mechanism to training?

A useful bounded comparison uses one deterministic training loop for an uninterrupted reference, a complete application save/restart, and a process snapshot/restore. After update 20, compare restored model and optimizer tensors, step, data position, and random-generator state **before** update 21. Then compare updates 21–25 against the reference. Confirm the two runs also matched at update 20 before capture.
...
Measure request-to-GPU-release, request-to-completed-image, restore-to-next-completed-update, image size, and peak host RAM. Demonstrate that a second small job can use the released GPU and exit before restoring the first. Require the original process to end after image finalization for the full-process claim. Use the same cached assets, pause boundary, and storage criterion for the application-checkpoint comparison.

These are learning outcomes, not a new implementation plan or claims that training acceptance already passed. The current [results](../experiments/results.md) distinguish the blocked CRIU route, successful NVIDIA-only pause/resume, and qualified DMTCP full-process restore. Complete LoRA continuation remains untested; workload research is in [single-GPU fine-tuning](../research/single-gpu-finetuning.md).

...
Current evidence: local and remote workload/checker tests pass. CRIU 3.16.1 remains blocked during startup feature detection. NVIDIA-only GPU suspension passed. Subsequent DMTCP experiments restored both the CPU counter and a complete PyTorch GPU process after original exit; tensor equality and subsequent GPU operation checks passed, but four anonymous shared-memory warnings qualify the GPU result. Full fine-tuning correctness and instance-loss recovery remain unverified. See `gate-results.md` and the [fine-tuning feasibility audit](r-2026-09-14-finetuning-feasibility.html) for the pinned DMTCP alternative and remaining gates.

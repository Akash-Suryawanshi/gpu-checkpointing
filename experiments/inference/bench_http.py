...
if __name__ == "__main__":
    ...
    probe = commands.add_parser("ready", help="Exit zero only if the endpoint identifies itself")
    probe.add_argument("--url", required=True)
    probe.add_argument("--model", default="qwen3-8b")
    probe.add_argument("--route", choices=("fresh", "snapshot"), required=True)
    probe.add_argument("--attempts", type=int, default=1)
    probe.set_defaults(func=lambda a: print(json.dumps(ready(a.url, a.model, a.route, a.attempts, 0.2))))
